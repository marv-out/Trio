...
final class BaseUserNotificationsManager: NSObject, UserNotificationsManager, Injectable {
    ...
    /// Keeps the app icon badge current: on GRDB, `glucoseStorage.updatePublisher`
    /// fires for every change (inserts and deletions), so it is the only source needed.
    ...
    @MainActor private func updateGlucoseBadge() async {
        do {
            ...
            let latest = try await fetchGlucose().first?.glucose
