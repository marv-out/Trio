...
///
/// Lookup flow:
///   1. CGM update → fetch the latest reading from GRDB.
...
final class GlucoseAlertCoordinator: Injectable {
    ...
    /// Two-stage: async-fetch the latest value from GRDB, then hop onto
    /// `evaluationQueue` to mutate `firingAlertIDs` safely.
    ...
    /// Async fetch from GRDB (glucose no longer lives in Core Data). Value types
    /// cross threads freely, so no context/perform dance is needed.
    private func fetchLatestReadingMgDL() async -> Decimal? {
        let cutoff = Date().addingTimeInterval(-Self.readingFreshnessWindow)
        do {
            let latest = try await GlucoseStore.fetch(from: cutoff, ascending: false, limit: 1).first
            return latest.map { Decimal($0.glucose) }
        } catch {
