...
extension Stat.StateModel {
    ...
    private func fetchMealStats() async throws -> (hourly: [MealStats], daily: [MealStats]) {
        // Fetch CarbEntryRecord entries from GRDB (newest first; sort ascending below to match old behaviour)
        let entries = try await CarbEntryStore.fetchForStats()
        // fetchForStats() returns newest-first; restore the previous ascending order so that
        // grouping/iteration produces the same result as the old Core Data fetch (ascending: true).
        let fetchedResults = entries.sorted { ($0.date ?? .distantPast) < ($1.date ?? .distantPast) }
